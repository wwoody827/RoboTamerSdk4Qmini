        ...
        while (running_) {
            ...
            for (int id : ids) {
                ...
                try {
                    ...
                    // Joint torque = motor torque × ratio (position/velocity
                    // reduce by the ratio, but torque amplifies). Previously
                    // this divided, making tau_est ~ratio² too small. Fixed
                    // and VERIFIED 2026-05-31: in the static-balance log
                    // d.tau×ratio matches the kp·err gravity torque per joint
                    // (docs/calibration_notes/static_balance_2026_05_31.md).
                    // tau_est is telemetry-only (not in obs/command path).
