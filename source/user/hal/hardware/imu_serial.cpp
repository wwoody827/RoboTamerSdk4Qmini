    ...
    void run() {
        ...
        while (running_) {
            ...
            if (type == kTypeImu) {
                ...
                float f[10];
                std::memcpy(f, payload, sizeof(f));
                // Axis remapping mirrors imu_receiver.py exactly.
                pending.omega[0] =  f[1];           // RollSpeed
                pending.omega[1] = -f[0];           // PitchSpeed (sign flip)
                pending.omega[2] =  f[2];           // HeadingSpeed
                pending.rpy[0]   =  f[4];           // Roll  = ahrs[4]
                pending.rpy[1]   = -f[3];           // Pitch = -ahrs[3]
                pending.rpy[2]   =  f[5];           // Yaw   = ahrs[5]
                pending.quat[0]  =  f[6];           // qw
